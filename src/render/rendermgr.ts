...
export class Rendermgr {
    ...
    renderScene(scene: Scene) {
        ...
        let rootTrans = scene.getRootTransforms();
        for (let i = 0; i < rootTrans.length; i++) {
            this._fillRenderer(rootTrans[i]);
        }
    ...
    private _fillRenderer(node: Transform) {
        if (node.entity.beActive) {
            let obj = node.entity;
            //物体rendercheck--》addtorenderlist--》list排序--》物体render---》真的draw
            if (obj.render != null) {
    ...
    draw(mesh: Mesh, mat: Material, submesh: subMeshInfo, localDrawType: DrawTypeEnum) {
        if (this.activeInstanceDrawType) {
            ...
            for (let i = 0; i < pass.program.length; i++) {
                ...
                webGraph.render.applyMatUniforms(
                    usingProgram,
                    ShaderVariant.AutoUniformDic,
                    mat.UniformDic,
                    mat.getShader().mapUniformDef,
                );
            ...
            if (mat == null || mat.getShaderPass(drawtype) == null) {
                mat =
                    mesh.vertexAttData[VertexAttTypeEnum.UV0] != null
                        ? assetMgr.getDefaultMaterial("def")
                        : assetMgr.getDefaultMaterial("defcolor");
            }
            let pass = mat.getShaderPass(drawtype);
            // if(pass==null)
            // {
            //     mat=mesh.vertexAttData[VertexAttTypeEnum.UV0]!=null?assetMgr.getDefaultMaterial("def"):assetMgr.getDefaultMaterial("defcolor");
            //     pass=mat.getShaderPass(drawtype);
            // }

            for (let i = 0; i < pass.program.length; i++) {
                let usingProgram = pass.program[i];
                // render.applyShaderProgram(usingProgram);
                webGraph.render.bindMeshAndProgram(mesh.glMesh, usingProgram);
                webGraph.render.applyMatUniforms(
                    usingProgram,
                    ShaderVariant.AutoUniformDic,
                    mat.UniformDic,
    ...
    private instanceRenderAll(cam: Camera, instanceList: { [matId: number]: IRender[] }) {
        ...
        if (this.instanceDataInit == false) {
            ...
            this.posAtt = webGraph.VertexAttribute.PrepareVertexAttribute(
                VertexAttTypeEnum.instance_pos,
                this.realPosDataArr,
            );
            this.rotAtt = webGraph.VertexAttribute.PrepareVertexAttribute(
                VertexAttTypeEnum.instance_rot,
                this.realRotDataArr,
            );
            this.scaleAtt = webGraph.VertexAttribute.PrepareVertexAttribute(
                VertexAttTypeEnum.instance_scale,
                this.realScaleDataArr,
            );
        }
    ...
    bindMat(mat: Material, drawType: DrawTypeEnum, programIndex: number = 0) {
        ...
        webGraph.render.applyMatUniforms(program, ShaderVariant.AutoUniformDic, mat.UniformDic, shader.mapUniformDef);
    }
