...
import { getFileName, getAssetDirectory } from "../base/helper";
...
export default class LoadGlTF implements IassetLoader
{
    ...
    ): Iasset
    {
        ...
        this.loadAsync(url)
            .then(gltfJson =>
            {
                ...
                ParseSceneNode.parse(scene, gltfJson, this.context).then(scene =>
                {
                    if (this.onFinish)
                    {
