...
export class Color extends Float32Array {
    ...
    public static create(r: number = 1, g: number = 1, b: number = 1, a: number = 1): Color {
        if (Color.Recycle && Color.Recycle.length > 0) {
            const item = Color.Recycle.pop() as Color;
            item[0] = r;
            item[1] = g;
            item[2] = b;
            item[3] = a;
            ...
            const item = new Color(r, g, b, a);
            return item;
        }
    }

    public static random(): Color {
        const item = new Color(Math.random() * 0.5 + 0.5, Math.random() * 0.5 + 0.5, Math.random() * 0.5 + 0.5, 1.0);
        return item;
    }
    ...
    public static clone(from: Color): Color {
        if (Color.Recycle.length > 0) {
            ...
            const item = new Color(from[0], from[1], from[2], from[3]);
            return item;
        }
    }
