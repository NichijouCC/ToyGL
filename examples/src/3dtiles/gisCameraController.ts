import { CameraComponent, Input, mat4, MouseKeyEnum, quat, Tiles3d, vec2, vec3, vec4, World, mat3 } from "TOYGL";
...
export class gisCameraController {
    world: World;
    constructor(world: World, system: Tiles3d.TilesetSystem) {
        this.world = world;
        this.initTranslateAndRotate(world, system);
    }

    viewTargetPoint(point: vec3, distance: number, angle: vec3 = vec3.fromValues(-45, 0, 0)) {
        let entity = this.world.mainCamera?.entity;
        if (entity == null) return;

        let gps = Tiles3d.ecefToWs84(point, vec3.create());
        let toEcefMat = Tiles3d.transformEnuToEcef(gps);

        let rot = quat.fromEuler(quat.create(), angle[0], angle[1], angle[2]);
        let forward = vec3.transformQuat(vec3.create(), vec3.FORWARD, rot);
        let camLocalPos = vec3.scale(vec3.create(), forward, distance);
        let localMat = mat4.fromRotationTranslation(mat4.create(), rot, camLocalPos);

        let worldMat = mat4.multiply(toEcefMat, toEcefMat, localMat);
        entity.worldMatrix = worldMat;
    }

    private initTranslateAndRotate(world: World, system: Tiles3d.TilesetSystem) {
